...
[Kalman_stable_result, inovace_KF_stable] = Kalman_filter_ust(trajektorie_cell, mereni_cell, pocet_kroku, F, G, H, Q, R, x0_cov, x0_mean, K_inf);
...
KF_stable_trajektorie_1 = Kalman_stable_result{1}; 
...
plot(0:length(KF_stable_trajektorie_1)-1, KF_stable_trajektorie_1(:, 1));
mereni_1(1) = NaN; 
...
plot(0:length(KF_stable_trajektorie_1)-1, KF_stable_trajektorie_1(:, 2));
...
chyba_KF_stable = trajektorie_1 - KF_stable_trajektorie_1; 
...
plot(0:length(KF_stable_trajektorie_1)-1, chyba_KF_stable(:, 1), 'Color', colorOrder(4,:));
...
plot(0:length(KF_stable_trajektorie_1)-1, chyba_KF_stable(:, 2), 'Color', colorOrder(4,:));
plot(0:length(KF_trajektorie_1)-1, zeros(1, length(KF_trajektorie_1)), 'b--');
...
plot(0:length(inovace_KF_stable(:,1))-1, inovace_KF_stable(:, 1), 'Color', colorOrder(4,:));
xlabel('k'); 
...
result_means_inovace_KF_stable = zeros(pocet_kroku+1, 1); 

for i = 1:pocet_kroku+1
    inovace_KF_ki = inovace_KF(i,:); 
    result_means_inovace_KF(i, 1) = mean(inovace_KF_ki);
    
    inovace_KF_stable_ki = inovace_KF_stable(i, :); 
    result_means_inovace_KF_stable(i, 1) = mean(inovace_KF_stable_ki); 
end

figure; 
hold on 
plot(0:length(result_means_inovace_KF)-1, result_means_inovace_KF(:, 1), 'Color', colorOrder(3,:));
plot(0:length(result_means_inovace_KF_stable)-1, result_means_inovace_KF_stable(:, 1), 'Color', colorOrder(4,:));
xlabel('k'); 
...
for k = vyber_kroky
    ...
    clen1_KF_stable = inovace_KF_stable(k, :); 
    ...
    for n = vyber_kroky
        ...
        clen2_KF_stable = inovace_KF_stable(n, :);  
        
        kovariance_KF = cov(clen1_KF, clen2_KF); 
        kovariance_KF_stable = cov(clen1_KF_stable, clen2_KF_stable); 
        
        kov_matice_inovace_KF(i, j) = kovariance_KF(2,1); % kovariance = [Px Pxz, Pzx Pz]
        kov_matice_inovace_KF_ust(i, j) = kovariance_KF_stable(2,1); % kovariance = [Px Pxz, Pzx Pz]
    end
end
...
for i = 1:pocet_trajektorii
    ...
    odhad_Kalman_ust = Kalman_stable_result{i}; 
